...
int sayHello(int fd, int sd, int nfds)
{
	...
	for (int w_fd = 0; w_fd <= nfds; w_fd++)
	{
		if (w_fd != sd && w_fd != fd)
		{
			bzero(msgrasp, 100);
			sprintf(msgrasp, "%d", fd); 
			strcat(msgrasp, ": ");
			strcat(msgrasp, msg);

			printf("[server]Trimitem mesajul inapoi catre %d...%s\n", w_fd, msgrasp);

			if (bytes && write(w_fd, msgrasp, bytes) < 0)
			{
